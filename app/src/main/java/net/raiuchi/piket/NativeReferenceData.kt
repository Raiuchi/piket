...
data class TimingStation(val name: String, val meters: Double, val displayMeters: Double = meters)
...
class NativeReferenceData(context: Context) {
    ...
    fun stationDisplayMeters(route: String, station: String, trainNumber: String? = null): Double? {
        val raw = normalizeStation(station)
        val wanted = stationAliases[raw] ?: raw
        val candidates = stations(route, trainNumber)
        return candidates.firstOrNull { normalizeStation(it.name) == wanted }?.displayMeters
            ?: candidates.firstOrNull {
                val candidate = normalizeStation(it.name)
                candidate.contains(wanted) || wanted.contains(candidate)
            }?.displayMeters
    ...
    private fun dachaThroughStations(): List<TimingStation> {
        ...
        val north = timingStations["Горы - Петрозаводск"].orEmpty()
        // Travel distance reaches old 34 km at Gory, then the official axis jumps to 42 km.
        // Keep travel meters continuous for time/speed math and display the official axis separately.
        return dacha + listOf(TimingStation("Горы", 34_000.0, 42_000.0)) +
            north.drop(1).map { TimingStation(it.name, it.meters - 8_000.0, it.meters) }
    }
